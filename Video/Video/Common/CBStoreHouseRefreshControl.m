...
#define COLOR [UIColor colorWithRed:5./255 green:174./255. blue:233./255. alpha:1.0]

#define STEP       0.03

static const CGFloat kloadingIndividualAnimationTiming = 0.8;
static const CGFloat kbarDarkAlpha = 0.6;
static const CGFloat kloadingTimingOffset = 0.12;
                          ...
                          internalAnimationFactor:(CGFloat)internalAnimationFactor
{
    ...
    for (int i=0; i<startPoints.count; i++) {
        ...
        if (endPoint.x > width) width = endPoint.x + 4;
...
+ (CBStoreHouseRefreshControl*)attachToView:(UIView *)view3
                                     target:(id)target
                              refreshAction:(SEL)refreshAction
                                      plist:(NSString *)plist
                                      color:(UIColor*)color
...
{
    if (self.originalTopContentInset == 0) self.originalTopContentInset = /*self.scrollView.contentInset.top;*/0;
...
- (void)scrollViewDidEndDragging
{
    if (self.state == CBStoreHouseRefreshControlStateIdle) {
        ...
        if (self.state == CBStoreHouseRefreshControlStateRefreshing) {
            ...
            UIEdgeInsets newInsets = /*self.scrollView.contentInset*/ UIEdgeInsetsZero;
            newInsets.top = self.originalTopContentInset + self.dropHeight;
            CGPoint contentOffset = /*self.scrollView.contentOffset*/CGPointMake(0, 0);
...
{
    return /*self.scrollView.contentOffset.y + self.originalTopContentInset*/0;
}
